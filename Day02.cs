namespace advent_of_code_2017
{
    ...
    public class Day02
    {
        private const string Problem1Input = @"1236	741	557	1029	144	101	1968	2159	1399	80	1139	1167	1695	82	90	2236
...
66	87	176	107	2791	109	21	92	3016	2239	1708	3175	3210	2842	446	484";
        ...
        {
            var rows = Utils.SplitToLines(Problem1Input);

            return rows.Select(GetMaxDifference).Sum();
        }

        private static int GetMaxDifference(string row)
        {
            var values = row.Split("\t").Select(Int32.Parse).ToList();
        ...
        public static int SolveProblem2()
        {
            ...
            return rows.Select(GetEvenDivisionResult).Sum();
        }

        private static int GetEvenDivisionResult(string row)
        {
            var values = row.Split("\t").Select(Int32.Parse).ToList();

            return GetEvenDivisionResult(values);
        }

        private static int GetEvenDivisionResult(List<int> values)
        {
            ...
            throw new Exception("Couldn't find even division");
        }
    }
}
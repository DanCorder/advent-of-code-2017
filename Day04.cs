namespace advent_of_code_2017
{
    ...
    public class Day04
    {
        private const string Problem1Input = @"pphsv ojtou brvhsj cer ntfhlra udeh ccgtyzc zoyzmh jum lugbnk
...
juo pmiyoh xxk myphio ogfyf dovlmwm moevao qqxidn";
        ...
        public static int SolveProblem1()
        {
            ...
            foreach(var passphrase in rows)
            {
                var words = passphrase.Split(' ');
                var isValid = words.GroupBy(x => x).Where(group => group.Count() > 1).Count() == 0;
        ...
        public static int SolveProblem2()
        {
            ...
            {
                var words = passphrase.Split(' ');
                var isValid = words.Select(x => String.Concat(x.OrderBy(c => c))).GroupBy(x => x).Where(group => group.Count() > 1).Count() == 0;

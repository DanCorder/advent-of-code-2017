namespace advent_of_code_2017
{
    ...
    public class Day07
    {
//         private const string Problem1Input = @"pbga (66)
// xhth (57)
// ebii (61)
// havc (66)
// ktlj (57)
...
// gyxo (61)
// cntj (57)";
        private const string Problem1Input = @"mqdjo (83)
...
koklf (61) -> jqtbsy, eetpan, vamssbi, rvwvokn, vpjsxth";
        ...
        public static string SolveProblem1()
        {
            ...
            var allProgramsByName = new Dictionary<string, Program>();
            ...
            foreach (var line in lines)
            {
                ...
                Program prog = allProgramsByName.ContainsKey(progName) ?
                    allProgramsByName[progName] : new Program { Name = progName };
                allProgramsByName[progName] = prog;

                prog.Weight = Int32.Parse(Match.Groups[2].Value);

                if (Match.Groups[3].Value != "")
                {
                    var names = Match.Groups[3].Value.Substring(3).Split(',').Select(s => s.Trim()).ToList();

                    foreach (var name in names)
                    {
                        var supportedProg = allProgramsByName.ContainsKey(name) ?
                            allProgramsByName[name] : new Program { Name = name };
                        allProgramsByName[name] = supportedProg;

                        prog.Supporting.Add(supportedProg);
                        supportedProg.SupportedBy = prog;
                    }
                }
            }

            return allProgramsByName.Single(p => p.Value.SupportedBy == null).Key;
        }

        private static Regex Parser = new Regex(@"(\w+) \((\d+)\)( -> ((\w+), )*(\w+))?");

        private static string GetProgram(string line)
        {
            var result = Parser.Match(line);

            return result.Captures[0].Value;
        }
        ...
        public static int SolveProblem2()
        {
            ...
            var baseProg =  allProgramsByName[allProgramsByName.Single(p => p.Value.SupportedBy == null).Key];

            GetTotalWeights(baseProg, allProgramsByName);

            var bad = CheckBalance(baseProg);

            var badSupporter = bad.SupportedBy;

            var goodWeight = badSupporter.Supporting.GroupBy(p => p.WeightPlusSupported)
                .Where(g => g.Count() > 1).Select(g => g.First().WeightPlusSupported).First();

            var diff = bad.WeightPlusSupported - goodWeight;

            return bad.Weight - diff;
        }

        private static void GetTotalWeights(Program baseProg, Dictionary<string, Program> programsByName)
        {
            var total = 0;
            foreach (var p in baseProg.Supporting)
            {
                GetTotalWeights(p, programsByName);
                total += p.WeightPlusSupported;
            }
            baseProg.WeightPlusSupported = baseProg.Weight + total;
        }

        private static Program CheckBalance(Program baseProg)
        {
            var unbalanced = baseProg.Supporting.GroupBy(p => p.WeightPlusSupported)
                .SingleOrDefault(g => g.Count() == 1);

            if (unbalanced != null)
            {
                return CheckBalance(unbalanced.First());
            }
        ...
        private class Program {
            ...
            public int WeightPlusSupported {get; set;} = -1;
            public List<Program> Supporting {get;} = new List<Program>();

            public Program SupportedBy {get;set;}
        }
    }
}
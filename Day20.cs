namespace advent_of_code_2017
{
    ...
    public class Day20
    {
        private const string Problem1Input = @"p=<-317,1413,1507>, v=<19,-102,-108>, a=<1,-3,-3>
...
p=<-2781,1254,-451>, v=<-399,186,-64>, a=<26,-13,0>";
        ...
        {
            var ret = Utils.SplitToLines(Problem1Input)
                .Select((row, index) => {
                    var segments = row.Split("=<").Select(x => x.Split('>')[0]).ToList();
                    return new {
                        i = index,
                        p = GetManhattanMagnitude(segments[1].Split(",").Select(Int32.Parse)),
                        v = GetManhattanMagnitude(segments[2].Split(",").Select(Int32.Parse)),
                        a = GetManhattanMagnitude(segments[3].Split(",").Select(Int32.Parse))
                        };
                })
                .OrderBy(x => x.a)
                .ThenBy(x => x.v)
        ...
        private static int GetManhattanMagnitude(IEnumerable<int> values)
        {
            return values.Select(Math.Abs).Sum();
        }


        public static int SolveProblem2()
        {
            var particles = Utils.SplitToLines(Problem1Input)
                .Select((row, index) => {
                    var segments = row.Split("=<").Select(x => x.Split('>')[0]).ToList();
                    var p = segments[1].Split(",").Select(Int32.Parse).ToArray();
                    var v = segments[2].Split(",").Select(Int32.Parse).ToArray();
                    var a = segments[3].Split(",").Select(Int32.Parse).ToArray();
                    return new Particle {
                            Position = Tuple.Create(p[0], p[1], p[2]),
                            Velocity = Tuple.Create(v[0], v[1], v[2]),
                            Acceleration = Tuple.Create(a[0], a[1], a[2])
                        };
                })
                .ToArray();
            ...
            do {
                ...
                Move(particles);
                i++;
            } while (CollisionsPossible(particles) && i < 100000);

            return particles.Length;
        }

        private static void Move(Particle[] particles)
        {
            foreach (var p in particles)
            {
                p.Velocity = Tuple.Create(
                    p.Velocity.Item1 + p.Acceleration.Item1,
                    p.Velocity.Item2 + p.Acceleration.Item2,
                    p.Velocity.Item3 + p.Acceleration.Item3);

                p.Position = Tuple.Create(
                    p.Velocity.Item1 + p.Position.Item1,
                    p.Velocity.Item2 + p.Position.Item2,
                    p.Velocity.Item3 + p.Position.Item3);
            }
        }

        private static bool CollisionsPossible(Particle[] particles)
        {
            if (!particles.All(MovingAwayFromOrigin))
            {
                return true;
            }
            //qq:DCC check more.
        ...
        private static bool MovingAwayFromOrigin(Particle p)
        {
            return (((p.Velocity.Item1 >= 0 && p.Acceleration.Item1 >= 0 && p.Position.Item1 >=0)
                || (p.Velocity.Item1 <= 0 && p.Acceleration.Item1 <= 0 && p.Position.Item1 <=0))
                && ((p.Velocity.Item2 >= 0 && p.Acceleration.Item2 >= 0 && p.Position.Item2 >=0)
                || (p.Velocity.Item2 <= 0 && p.Acceleration.Item2 <= 0 && p.Position.Item2 <=0))
                && ((p.Velocity.Item3 >= 0 && p.Acceleration.Item3 >= 0 && p.Position.Item3 >=0)
                || (p.Velocity.Item3 <= 0 && p.Acceleration.Item3 <= 0 && p.Position.Item3 <=0)));
        }

        private static Particle[] RemoveCollisions(Particle[] particles)
        {
            return particles

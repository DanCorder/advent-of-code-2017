namespace advent_of_code_2017
{
    ...
    public class Day08
    {
        private const string Problem1Input = @"ioe dec 890 if qk > -10
gif inc -533 if qt <= 7
itw dec 894 if t != 0
...
ih dec 369 if ih == 1993";
        ...
        {
            var instructions = Utils.SplitToLines(Problem1Input);
            var registers = new Dictionary<string, int>();

            foreach (var instruction in instructions)
            {
                ProcessInstruction(instruction, registers);
            }

            return registers.Select(x => x.Value).Max();
        }

        private static void ProcessInstruction(string instruction, Dictionary<string, int> registers)
        {
            var parts = instruction.Split(' ');

            var testRegisterName = parts[4];
            var testRegisterValue = GetRegisterValue(testRegisterName, registers);
            var testInstrucitonValue = Int32.Parse(parts[6]);
            bool doInstruction = false;

            switch (parts[5])
            {
                case "==":
                    doInstruction = testRegisterValue == testInstrucitonValue;
                    break;
                case "!=":
                    doInstruction = testRegisterValue != testInstrucitonValue;
                    break;
            ...
            var registerValue = GetRegisterValue(registerName, registers);
        ...
        private static int GetRegisterValue(string name, Dictionary<string, int> registers)
        {
        ...
        public static int SolveProblem2()
        {
            ...
            {
                ProcessInstruction(instruction, registers);
                max = Math.Max(max, registers.Select(x => x.Value).Max());
            }
